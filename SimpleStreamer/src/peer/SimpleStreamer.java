	...
	public static void main(String[] args) throws CmdLineException {
		...
		for (int i = 0; i < hosts.length; i++) {
			...
			try {
				...
				Thread connect_to_peer = new Thread(new Peer(socket, rate, width, height, peer_no, "CLIENT"));
		...
		try {
			...
			while (true) {
				...
				// Thread new Peer
				Thread receive_connecting_peer = new Thread(new Peer(socket, rate, width, height, peer_no, "SERVER"));
